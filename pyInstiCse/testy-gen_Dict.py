...
def addSaltNewPword():
    with open('BruteLowerCaseDigit.txt','r+') as sourceFile: 
        with open('hash6.txt','a+') as appendFile:
            # since i am the one created the source file, i know its formating 
            # so extracting directing
            sourceFile.seek(0)
            messageLine = sourceFile.readlines()
            messageLine.pop(0) # removes the header
            messageLine.pop(-1) # removes the time taken sentence
            for line in messageLine:
                print(line[13:18])
                appendFile.write(line[13:18] + '\n')
